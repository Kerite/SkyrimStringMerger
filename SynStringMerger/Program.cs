...
public class Program
{
    ...
    private static Regex isFullEnglishRegex = new Regex(@"^[\x00-\xff]+$", RegexOptions.Compiled);
    private static Regex isFullUpperCamelCase = new Regex(@"^([A-Z]+[a-z0-9]+)+$", RegexOptions.Compiled);
        ...
        where TTarget : class, IMajorRecordInternal, ITranslatedNamed, TCache
    {
        int i = 0;
        // 遍历 FormList
        foreach (var translating in Settings.PatchAllRecords ? sourceModEntities : patchedModsEntities)
        {
            ...
            {
                // 全是中文或者是大驼峰
                if (Settings.SkipEnglishString && isFullEnglishRegex.IsMatch(nameSource.Name.String)
